#!/usr/bin/env python3
"""
LightGBM v2 Models — No normalization, calibration support.
"""
...
MONEYLINE_V2_PATH = DATA_DIR / "lgb_moneyline_v2.pkl"
TOTALS_V2_PATH = DATA_DIR / "lgb_totals_v2.pkl"
SPREAD_V2_PATH = DATA_DIR / "lgb_spread_v2.pkl"


class LGBMoneylineModelV2(BaseModel):
    """LightGBM v2 classifier for moneyline — no normalization, optional calibration."""

    name = "lgb_moneyline_v2"
    version = "2.0"
    description = "LightGBM v2 gradient boosting for moneyline (no norm, calibrated)"
    ...
    def _load_model(self):
        ...
        if MONEYLINE_V2_PATH.exists():
            try:
                ...
                print(f"Warning: Could not load LightGBM v2 moneyline model: {e}")
...
class LGBTotalsModelV2(BaseModel):
    """LightGBM v2 regressor for total runs — no normalization."""

    name = "lgb_totals_v2"
    version = "2.0"
    description = "LightGBM v2 for run totals (no norm)"
    ...
    def _load_model(self):
        ...
        if TOTALS_V2_PATH.exists():
            try:
                ...
                print(f"Warning: Could not load LightGBM v2 totals model: {e}")
...
class LGBSpreadModelV2(BaseModel):
    """LightGBM v2 regressor for spread — no normalization."""

    name = "lgb_spread_v2"
    version = "2.0"
    description = "LightGBM v2 for spread (no norm)"
    ...
    def _load_model(self):
        ...
        if SPREAD_V2_PATH.exists():
            try:
                ...
                print(f"Warning: Could not load LightGBM v2 spread model: {e}")
