    ...
    try:
        ...
        for row in rows:
            ...
            grouped.setdefault(row["model_name"], []).append(
                (float(row["predicted_home_prob"]), int(row["home_won"]))
            )
        return grouped
    finally:
        conn.close()
...
def generate_report():
    ...
    lines.append("")
    lines.append("| Model | N | Brier | LogLoss | Avg Pred | Actual Home Win Rate | |Gap| |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|")
    ...
    for model_name, rows in sorted(grouped.items()):
        ...
        n = len(rows)
        avg_pred = sum(p for p, _ in rows) / n
        actual_rate = sum(y for _, y in rows) / n
        gap = abs(actual_rate - avg_pred)
        summaries.append((model_name, n, brier, logloss, avg_pred, actual_rate, gap))

    for model_name, n, brier, logloss, avg_pred, actual_rate, gap in sorted(summaries, key=lambda x: (-x[1], x[0])):
        lines.append(
            f"| {model_name} | {n} | {brier:.4f} | {logloss:.4f} | {avg_pred:.4f} | {actual_rate:.4f} | {gap:.4f} |"
        )
    ...
    for model_name, rows in sorted(grouped.items(), key=lambda item: (-len(item[1]), item[0])):
        ...
        lines.append("|---|---:|---:|---:|---:|")
        for b in _reliability_bins(rows):
            lines.append(
